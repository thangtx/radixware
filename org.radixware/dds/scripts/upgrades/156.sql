...
create or replace package RDX_Array as
	...
	function toStrTable(
		...
		-- Length to crop for array content. Need be in the range 0..4000. 0 means no cropping
		cropLength in integer
	) return RDX_STR_TABLE;
end;
/

grant execute on package RDX_Array to &USER&_RUN_ROLE
/
...
create or replace package body RDX_Array as
	...
	begin
	  ...
	  if lob is null then
	    ...
	    RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal softMode parameter value ('||to_char(softMode)||')! Only 0 and 1 are valid');
	  elsif not cropLength between 0 and 4000 then
	    RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal cropLength parameter value ('||to_char(cropLength)||')! Only 0..4000 is valid');
	  end if;
	  ...
	  loop
	    ...
	    if (b = 0) then 
	      if softMode = 1 then
	          ...
	          if (i - 1 < sz) then
	            RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal array content! Array contains less elements ('||to_char(i-1)||') than were declared in the array header ('||to_char(sz)||'), and softMode = 0');
	          elsif e > 0 then 
	            RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal array content! Unpaired brackets at the '||to_char(i-1)||'-th element of the array content, and softMode = 0');
	          else
	            return res; 
	          end if;
	      end if;
	    ...
	    if e = 0 then
	      if softMode = 1 then
	          ...
	          RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal array content! Unpaired brackets at the '||to_char(i)||'-th element of the array content, and softMode = 0');
	      end if;
	    elsif e < b then
	      if softMode = 1 then
	          return res; 
	      ...
	      if cropLength = 0 then
	        if len > 4000 then
	          RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal array content! The '||to_char(i)||'-th element of array is more than 4000 chars');
	        else
	          croppedLen := len;
	        end if;
	      ...
	      if (i > sz) then 
	        if softMode = 1 then
	            ...
	            RAISE_APPLICATION_ERROR (-20100, 'RDX_Array.toStrTable: illegal array content! Array contains more elements ('||to_char(i)||') than were declared in the array header ('||to_char(sz)||'), and softMode = 0');
	        end if;
	      end if;
	...
	-- select * from table(Rdx_Array.toStrTable('3[3]100[2]AS',1,0)) -- 2 lines: 100, AS
	...
	-- select * from table(Rdx_Array.toStrTable('2[3]100[2AS',1,0)) -- 1 line: 100
