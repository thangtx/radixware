...
final class FilePathStoreLastPathSetting extends AbstractCheckableSetting implements IEditMaskEditorSetting {
    
    public FilePathStoreLastPathSetting(final IClientEnvironment environment, final QWidget parent){
        super(parent);
        final String title = environment.getMessageProvider().translate("EditMask", "Store last path in config");
        this.setText(title);
    ...
    public EEditMaskOption getOption() {
        return EEditMaskOption.FILEPATH_FILE_STORE_PATH;
    }

    @Override
    public void addToXml(EditMask editMask) {
        editMask.getFilePath().setStoreLastPath(this.isChecked());
    }

    @Override
    public void loadFromXml(EditMask editMask) {
        final org.radixware.schemas.editmask.EditMaskFilePath ems = editMask.getFilePath();
        if (ems.isSetStoreLastPath()) {
            this.setChecked(ems.getStoreLastPath());
        } else {
            setDefaultValue();
        }
    }
