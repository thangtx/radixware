...
public abstract class ImageAcquiringDevice {  
    
    protected ImageAcquiringDevice(){        
    }
    ...
    public final File loadImageToFile(final String filePath, final boolean showImageConfirmationDialog){
        return ImageAcquiringDevice.this.loadImageToFile(filePath, showImageConfirmationDialog, null);
    }
    
    public final File loadImageToFile(final boolean showImageConfirmationDialog){
        return ImageAcquiringDevice.this.loadImageToFile(null, showImageConfirmationDialog, null);
    }
    
    private static AbstractIadBackend getBackend(final EImageAcquiringDeviceType type){
        return AbstractIadBackend.Factory.getInstance(type);
    }
    
    public static boolean isSupportedPlatform(final EImageAcquiringDeviceType type){
        return getBackend(type)!=null;
    }
    
    public static Collection<ImageAcquiringDevice> enumDevices(final IClientEnvironment environment, 
    ...
    public static ImageAcquiringDevice selectDevice(final IClientEnvironment environment, 
                                                    final EImageAcquiringDeviceType type,
    ...
    private static void showUnsupportedMsg(final IClientEnvironment environment, final EImageAcquiringDeviceType type){
        if (type==EImageAcquiringDeviceType.SCANNER && SystemTools.isOSX){
    ...
    public static QImage getImage(final IClientEnvironment environment, 
        ...
        final ImageAcquiringDevice device = selectDevice(environment, type, parentWidget, false);
    ...
    public static void release(){
        for (EImageAcquiringDeviceType type: EImageAcquiringDeviceType.values()){
            final AbstractIadBackend backend = getBackend(type);
            if (backend!=null){
                backend.release();
            }
