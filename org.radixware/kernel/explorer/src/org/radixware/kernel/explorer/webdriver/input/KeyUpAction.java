...
import static org.radixware.kernel.explorer.webdriver.input.KeyAction.getTargetWidget;

final class KeyUpAction extends KeyAction {    
    
    public KeyUpAction(final KeyInputSource inputSource, final char key){
    ...
    public void dispatch(final WebDrvSession session, int tickDuration, long tickStart) {
        ...
        if (targetWidget!=null && getQtKey()!=Qt.Key.Key_unknown){
            ...
            if (pressed){
                state.removePressedKey(getQtKey());
                final QKeyEvent event = 
                    new QKeyEvent(QEvent.Type.KeyRelease, getQtKey().value(), state.getModifiers(), getKeySymbol(), pressed);
