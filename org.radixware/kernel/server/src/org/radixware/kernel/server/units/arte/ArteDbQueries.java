...
final class ArteDbQueries extends AbstractDbQueries {

    private final ArteUnit unit;
    private ReadSapOptionsQuery readSapOptsQry = null;
    
    private PreparedStatement qryReadOptions = null;
    private static final String qryReadOptionsSQL = "select u.higharteinstcount, u.sapid, u.threadPriority from rdx_arteunit u where u.id = ?";
    
    private PreparedStatement qryWriteBasicStats = null;
    private static final String qryWriteBasicStatsSQL = "update rdx_arteunit set avgActiveArteCount=? where id = ?";
    
    private ArteDbQueries() {
        this(null);
    }
    
    ArteDbQueries(final ArteUnit unit) {
        this.unit = unit;
    }
    ...
    void writeBasicStats() throws SQLException {
        if (unit.getDbConnection() != null) {
            if (qryWriteBasicStats == null) {
                qryWriteBasicStats = unit.getDbConnection().prepareStatement(qryWriteBasicStatsSQL);
