    ...
    public final static void objVal2AasXmlVal(final Object val, final Value xmlValue) {
        ...
        try {
            ...
            if (val instanceof XmlObject) {
                ...
                final boolean xmlDocument = xml.getClass().getName().endsWith(XML_BEANS_DOCUMENT_CLASS_POSTFIX);
                final boolean xmlDocFragment = xml.getDomNode() != null && xml.getDomNode().getNodeType() == Node.DOCUMENT_FRAGMENT_NODE;
                if (xmlDocument || xmlDocFragment) { //документ, берем 1-го child-a (все равно без имени не копируется)
