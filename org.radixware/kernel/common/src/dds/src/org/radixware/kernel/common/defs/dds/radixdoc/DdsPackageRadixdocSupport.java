...
public class DdsPackageRadixdocSupport extends DdsDefinitionRadixdoc<DdsPackageDef> {
    ...
    public DdsPackageRadixdocSupport(DdsPackageDef source, Page page, DocumentOptions options) {
        ...
        for (DdsPlSqlObjectItemDef item : source.getHeader().getItems()) {
            if (item instanceof DdsPrototypeDef) {
                ...
                if (funcDef.isPublic() && funcDef.isGeneratedInDb()) {
                    ...
                    DdsFunctionDef function = ((DdsPrototypeDef) item).getFunction();
                    if (function.getDescriptionId() != null || function.getDescription() != null && !function.getDescription().isEmpty()) {
                        if (function.getDescriptionId() != null && function.findExistingLocalizingBundle() != null) {
                            getWriter().addMslId(descriptionCell, function.findExistingLocalizingBundle().getId(), function.getDescriptionId());
                        } else {
                            getWriter().addText(descriptionCell, function.getDescription());
                        }
                    }
                }
            }
