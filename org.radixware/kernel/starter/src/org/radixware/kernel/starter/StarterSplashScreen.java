...
public class StarterSplashScreen {
    ...
    public static void hideImpl() {
        final SplashWindow splashWindowSnapshot = splashWindow;
        if (splashWindowSnapshot != null) {
            SwingUtilities.invokeLater(new Runnable() {
                @Override
                public void run() {
                    splashWindowSnapshot.setVisible(false);
                    splashWindowSnapshot.dispose();
                    Frame frameSnapshot = frame;
                    if (frameSnapshot != null) {
                        frameSnapshot.dispose();
                    }
                    Thread updaterThreadSnapshot = statusUpdaterThread;
                    if (updaterThreadSnapshot != null) {
                        updaterThreadSnapshot.interrupt();
                    }
                    splashWindow = null;
                    frame = null;
    ...
    private static final class SplashWindow extends Window {
        ...
        private static final int TITLE_TOP_LEFT_X = 2;
        ...
        private void drawText(Graphics g) {
            font = g.getFont().deriveFont(4);
           
            g.setColor(TEXT_COLOR);
            g.setFont(font);
            g.drawString(text, TITLE_TOP_LEFT_X, (int) g.getFontMetrics().getStringBounds(text, g).getHeight());
