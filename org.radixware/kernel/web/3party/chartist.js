...
(function (window, document, Chartist) {
  ...
  Chartist.getHighLow = function (data, options, dimension) {
    ...
    var highLow = {
        roundFactorMillis: options.roundFactorMillis,
   ...
   */
    Chartist.getBounds = function (axisLength, highLow, scaleMinSpace, referenceValue, onlyInteger, isTimeAxis) {
    var i,
    ...
    bounds.values = [];
    if(isTimeAxis) {
         bounds.values = Chartist.TimeAxisUtils.getTimesTicks(bounds, axisLength);
         bounds.min = bounds.values[0] = bounds.low;
         bounds.max = bounds.values[bounds.values.length - 1] = bounds.high;
         bounds.range = bounds.max -  bounds.min;
      ...
      if(onlyInteger && Chartist.projectLength(axisLength, 1, bounds) >= scaleMinSpace) {
        ...
        while (true) {
          ...
          scaleUp = bounds.step < scaleMinSpace;
...
;/**
 * Time axis handling from flot.js library.
 *
 * @module Chartist.TimeAxisUtils
 */
/* global Chartist */
(function (window, document, Chartist) {
    'use strict';
    ...
    function getTimesTicks(bounds, axisLength) {
        var opts = options.xaxis;
        var axis = {
            min: bounds.low,
            max: bounds.high,
            delta: (bounds.high - bounds.low) / (0.3 * Math.sqrt(axisLength + 60)) //empiric value for ticks count
        };
        var ticks = [];
    ...
    var getTickLabel = function (v, timeOptions) {
	var d = dateGenerator(v, timeOptions);
	return formatDate(d, timeOptions.timeformat, timeOptions.monthNames, timeOptions.dayNames);
    }
    
    Chartist.TimeAxisUtils = {
        getTimesTicks : getTimesTicks,
...
(function (window, document, Chartist) {
  ...
  function AutoScaleAxis(axisUnit, data, chartRect, options) {
    ...
    this.bounds = Chartist.getBounds(chartRect[axisUnit.rectEnd] - chartRect[axisUnit.rectStart], highLow, options.scaleMinSpace || 20, options.referenceValue, options.onlyInteger, options.isTimeAxis);
    this.range = {
      min: this.bounds.min,
      max: this.bounds.max
    };
  ...
  function createChart(options) {
    ...
    data.raw.series.forEach(function(series, seriesIndex) {
      ...
      if(seriesOptions.showArea && axisY.range) {
        ...
        //***MY FIX*** Не рисуем зону, если она не вмещается на графике
        if(areaBase >= axisY.range.max) {
          for(var i = 0; i < series.data.length; i++) {
            if(series.data[i].y >= axisY.range.max) {
              return;
            }
          }
        }
