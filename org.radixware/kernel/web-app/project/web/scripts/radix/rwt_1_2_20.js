            ...
            case document.ELEMENT_NODE:
                ...
                if (node && node.attributes && node.attributes.length > 0) {
                    for (i = 0, il = node.attributes.length; i < il; i++) {
                        ...
                        var name = attr.name;
                        if (name === 'style') {
                            newNode.style.cssText = attr.value;
                        } else if (name === 'class') {
                            newNode.className = attr.value;
                        } else {
                ...
                if (parent != null && parent.className.indexOf('rwt-html-display-component') >= 0) {
                    ...
                    try {
                        ...
                        if (stub != null && stub.length > 0) {
                            for (var n = 0; n < stub.length; n++) {
                                if (stub[n].nodeType == document.ELEMENT_NODE) {
                                    return stub[n];
                                }
                            }
                            return stub[0];
...
function rdx_ajax_impl(rq_r, async) {
    ...
    function loadScript(scriptName, view, flow, rootId, rq) {
        ...
        script.rwt_done = function(e) {
            ...
            if (this.flow.script_count == 0) {
                doProcessResponse(view, this.rwt_rq, this.rwt_callback, rq);
            }
        };
        ...
        script.src = src;
        // if(rootId!=null)
        //script.src+='?root='+rootId;

        head.appendChild(script);
        return true;
    }
    function loadCss(scriptName, view, rootId) {
        var head = document.getElementsByTagName('head')[0];
    ...
    function saveLayouts(update, layouts) {
        WpsUtils.iterateNames(update, 'rwt-layout', function(l) {
            var id = $(l).attr('id');
            var func = $(l).attr('func');
            if (id && func) {
                layouts.push({
                    lid: id,
                    lfunc: func
    ...
    function updateObject(id, updates, layouts) {
                    ...
                    if (node != null) {
                        ...
                        WpsUtils.iterateAll(update, function(e) {
                            ...
                            if ('style-change' === partName) {
                                ...
                                if (type === 'remove') {
                                    $(node).css($(e).attr('name'), '');
                                } else {
                                    $(node).css($(e).attr('name'), $(e).attr('value'));
                                }
                        ...
                        for (var a = 0; a < u_remove.length; a++) {
                            ...
                            if (childNode) {
                                ...
                                if (childNode.ui_dialog) {
                                    ...
                                    //                                    
                                    //                                    $(childNode.ui_dialog_body).dialog('destroy');                                        
                                    //                                    $(childNode.ui_dialog_body).remove();
                                }
                            }
                            //childNode = document._findNodeById(childId);
                            //if(childNode)
    ...
    function doProcessResponse(view, rqId, callback, srcrq) {
        try {
            ...
            try {
                ...
                if (bodyId != null) {//we should start from body
                    updateObject(bodyId, updates, layouts);
                    uc++;
                }
                for (var id in updates) {
                    updateObject(id, updates, layouts);
                    uc++;
                }
            } finally {
    ...
    function processResponse(xml, rqId, callback, rq) {
        ...
        try {
            ...
            if (scriptFlow.script_count === 0) {
                doProcessResponse(view, rqId, callback, rq);
            } else {
                var someScriptIsNew = false;
                for (var idx in scripts) {
                    var obj = scripts[idx];
                    if (obj.t === 'j') {
                        if (loadScript(obj.v, view, scriptFlow, rootId, rq)) {
    ...
    function createWaitBox() {
        ...
        if (textbox == null || textbox.length == 0) {
            $('body').append('<table id="wps_sys_waitbox_text" class="ui-corner-all rwt-gradient-bar rwt-ui-shadow" style="display:none;color:white;position:absolute;z-index:99999999;">' +
                    '<tr>' +
                    '<td>' +
            ...
            //            var tb = textbox[0];
            //            var img = WpsUtils.createDocumentElement('img', {
            //                'src':'images/ajax-loader.gif'
            //            });
            //            tb.appendChild(img);
            //            var label = WpsUtils.createDocumentElement('label', {
            //                'id':'wps_sys_waitbox_text_label',
            //                'style':'color:white'
            //            });
    ...
    function processResponseAsync(rs) {
        processResponse(rs.responseXML, rs.rwt_id, rs.rwt_callbackFunc, rs);
    }
    ...
    try {
            ...
            if (this.readyState == 1) {
                ...
                try {
                    if (this.status == 200 || this.status == 'complete') {
                        ...
                        processResponseAsync(this);
                    } else {//server is not respnding
                        resetRqs();
                        destroyWaitBox();
                        //spliceRq(this.rwt_rq);
                    }
                } finally {
                }
            }
        ...
        persistentCookie = $RWT.cookies.filter(['JSESSIONID', 'll', 'ln', 'ls']);
        rq.send(null);
    } catch (e) {
        alertError(e);
    }
...
//    }, false);
//};
window.onload = $RWT._initSession;
//window.onbeforeunload = $RWT._killSession;
...
$RWT.events = {
    ...
    _event: function(element, eventName, details, params, callback) {
        ...
        $RWT.events._event_id(element.id, eventName, details, params, callback, priority);
    },
    _event_id: function(elementid, eventName, details, params, callback, priority) {
        ...
        $RWT.events.deffered_changes = null;
        rdx_ajax(q, callback, priority, eventName === "timer_tick");
    },
    ...
    _client_event: function(element, eventName, details, params) {
        ...
        $RWT.events._client_event_id(element.id, eventName, details, params);
    },
    ...
    _onUITick: function() {
        try {
            if ($RWT.timer.uiTicks) {
                ...
                setTimeout($RWT.timer._onUITick, 2000);
            }
        } catch (e) {
...
$RWT.waitWindow = {
    ...
    processResponse: function(xml) {
        ...
        var command = xml.getAttribute('command');
        if (command == 'ping') {//ignore ping command
...
$RWT.jsInvoker = {
    processResponse: function(xml) {
        ...
        return;
        $RWT.actions.event('jsInvoker', 'return', result);
    },
    _sendError: function(error) {
