...
public class ConstValueVTag<T extends ConstValueTag> extends SqmlVTag<T> {

    public ConstValueVTag(T tag) {
        super(tag);
    }
    ...
    protected void printTitle(CodePrinter cp) {
        ...
        if (enumDef != null) {
            cp.print(enumDef.getName());
        } else {
            cp.printError();
        }
        cp.print("::");
        final IEnumDef.IItem item = tag.findEnumItem();
        if (item != null) {
