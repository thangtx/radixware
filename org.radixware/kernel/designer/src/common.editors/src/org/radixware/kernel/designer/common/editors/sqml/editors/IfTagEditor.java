...
public class IfTagEditor<T extends IfParamTag> extends TagEditor<T> {
    ...
    private void updateValPanel() {
        ...
        Definition def = dlParam.getDefinition();
        ...
        if (def instanceof AdsPropertyDef) {
            final AdsPropertyDef property = (AdsPropertyDef) def;
            valType = property.getValue().getType().getTypeId();
        } else  if (def instanceof AdsFilterDef.Parameter) {
            final AdsFilterDef.Parameter parameter = (AdsFilterDef.Parameter) def;
            valType = parameter.getType().getTypeId();
        } 
    ...
    protected void installParams(SqmlEditorPanel codeEditor, Definition parameter) {
        if (parameter instanceof AdsFilterDef.Parameter) {
            final AdsFilterDef.Parameter p = (AdsFilterDef.Parameter) parameter;
            AdsFilterDef filter = p.getOwnerFilter();
            dlParam.setComboBoxValues(filter.getParameters().list(), false);
        }
    }

    @Override
    protected void afterOpen() {
        final Definition parameter = getParameter();
        
        final SqmlEditorPanel codeEditor = getOpenInfo().getLookup().lookup(SqmlEditorPanel.class);
        if (codeEditor != null) {
            dlParam.setComboMode();
            installParams(codeEditor, parameter);
        }
        dlParam.open(parameter, getObject().getParameterId());
        ...
        EValType valType = EValType.STR;
        
        if (parameter instanceof AdsPropertyDef){
            valType = ((AdsPropertyDef)parameter).getValue().getType().getTypeId();
        } else if (parameter instanceof AdsFilterDef.Parameter){
            valType = ((AdsFilterDef.Parameter)parameter).getType().getTypeId();
        }
    ...
    private void updateOperators() {
        ...
        EValType valType = EValType.STR;
        if (def instanceof AdsPropertyDef || def instanceof AdsFilterDef.Parameter) {
            parameter = def;
            if (parameter instanceof AdsPropertyDef) {
                valType = ((AdsPropertyDef) parameter).getValue().getType().getTypeId();
            } else if (parameter instanceof AdsFilterDef.Parameter) {
        ...
        if (parameter != null) {
            for (Operator operator : Operator.values()) {
                if (valType.isArrayType() == operator.isMulty()) {
    ...
    private Definition getParameter() {
        ...
        if (result == null) {
            result = getOpenInfo().getLookup().lookup(AdsFilterDef.Parameter.class);
            if (result == null) {
                result = getOpenInfo().getLookup().lookup(AdsParameterPropertyDef.class);
            }
            if (result == null) {
                result = getOpenInfo().getLookup().lookup(AdsDynamicPropertyDef.class);
            }
        }
    ...
    protected org.radixware.kernel.designer.common.dialogs.components.DefinitionLinkEditPanel dlParam;
