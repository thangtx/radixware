 ...
 */
class JmlTokenBalance implements TokenHierarchyListener {

    public static <T extends TokenId> JmlTokenBalance get(Document doc) {
        JmlTokenBalance tb = (JmlTokenBalance) doc.getProperty(JmlTokenBalance.class);
        if (tb == null) {
            tb = new JmlTokenBalance(doc);
            doc.putProperty(JmlTokenBalance.class, tb);
        }
        return tb;
    }
    ...
    private JmlTokenBalance(Document doc) {
        this.doc = doc;
