...
public class AddNewReportAction extends CookieAction {

    public static class Cookie implements Node.Cookie {

        private final ReportsModule module;

        public Cookie(final ReportsModule module) {
            this.module = module;
        }
        ...
        public void execute() {
            ProgressUtils.showProgressDialogAndRun(new Runnable() {
                ...
                @Override
                public void run() {
                    final UserReport report = UserExtensionManager.getInstance().getUserReports().addNewReport(module);
                    if (report != null) {
                        SwingUtilities.invokeLater(new Runnable() {

                            @Override
                            public void run() {
                                DialogUtils.goToObject(report.getVersions().getCurrent().findReportDefinition());
                            }
                        });
                    }
                }
