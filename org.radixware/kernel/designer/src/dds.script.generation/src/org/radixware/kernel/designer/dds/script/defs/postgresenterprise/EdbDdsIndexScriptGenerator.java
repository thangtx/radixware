...
public class EdbDdsIndexScriptGenerator implements IDdsDefinitionScriptGenerator<DdsIndexDef> {

    protected EdbDdsIndexScriptGenerator() {
    }
...
//        cp.println("-- INDEX: "+idx.getClass());
        ...
        if (!(idx instanceof DdsPrimaryKeyDef)) {
            ...
            cp.print(" index ").print(idx.getDbName()).print(" on ").print(tableDbName);

            String prefix = " (";
            
            for (DdsIndexDef.ColumnInfo columnInfo : idx.getColumnsInfo()) {
                final DdsColumnDef column = columnInfo.getColumn();
                
                cp.print(prefix).print(DdsScriptInternalUtils.escapeDbName(column.getDbName())).print(' ').print(columnInfo.getOrder().getValue().toLowerCase());
                prefix = ", ";
            }
            cp.print(')');
...
//        DdsIndexScriptGenerator generator = new DdsIndexScriptGenerator();
    ...
    public static final class Factory {
        ...
        public static EdbDdsIndexScriptGenerator newInstance() {
            return new EdbDdsIndexScriptGenerator();
        }
    }
