    ...
    private class SyncTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (modelColumnIndex == 0) {
                ...
                // RADIX
                String displayName = RadixFileUtil.getFileDisplayName(node.getFile());
                if (!isSelected) {
                    // RADIX
                    value = "<html>" + Subversion.getInstance().getAnnotator().annotateNameHtml(displayName, node.getFileInformation(), null); // NOI18N
                    // RADIX
                } else {
                    // RADIX
                    value = displayName;
                ...
                if (SvnModuleConfig.getDefault().isExcludedFromCommit(node.getFile().getAbsolutePath())) {
                    ...
                    if (isSelected) {
                        // RADIX
                        value = "<html><s>" + displayName + "</s></html>"; // NOI18N
                    } else {
                        // RADIX
                        value = "<html><s>" + Subversion.getInstance().getAnnotator().annotateNameHtml(displayName, node.getFileInformation(), null) + "</s>"; // NOI18N
                    }
                }
            }
