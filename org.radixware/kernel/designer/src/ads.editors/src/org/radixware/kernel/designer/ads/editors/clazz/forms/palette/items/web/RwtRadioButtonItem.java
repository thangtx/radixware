...
public class RwtRadioButtonItem extends Item {

    public static final RwtRadioButtonItem DEFAULT = new RwtRadioButtonItem();

    public RwtRadioButtonItem() {
        super(Group.GROUP_WEB_BUTTONS, "Radio Button", AdsMetaInfo.RWT_UI_RADIO_BUTTON);
    }

    @Override
    public RadixObject createObjectUI(RadixObject context) {
        AdsRwtWidgetDef widget = (AdsRwtWidgetDef) super.createObjectUI(context);
        widget.getProperties().add(new AdsUIProperty.LocalizedStringRefProperty("text", AdsUIUtil.createStringDef(context, "Radio", "Radio")));
        return widget;
    }
...
//        return new Dimension(Math.max(defaultSize.width, width + 2), Math.max(defaultSize.height, height + 2));
    ...
    public void paintWidget(Graphics2D gr, Rectangle r, RadixObject node) {
        AdsUIProperty.BooleanProperty checked = (AdsUIProperty.BooleanProperty) AdsUIUtil.getUiProperty(node, "selected");
