...
import org.radixware.kernel.explorer.webdriver.input.InputActions;

final class PerformActionsCmd extends GuiCmd{
    ...
    public JSONObject execute(final WebDrvSession session, final WebDrvUri uri, final JSONObject parameter) throws WebDrvException {
        ...
        final InputActions actions = InputActions.parse(parameter, session.getInputSourceManager());
        session.getInputSourceManager().dispatch(actions);
        return null;
    }
}
