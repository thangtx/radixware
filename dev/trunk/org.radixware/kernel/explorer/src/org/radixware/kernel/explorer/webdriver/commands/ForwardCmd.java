...
final class ForwardCmd extends NavigationCmd {
	...
	public JSONObject execute(WebDrvSession session, WebDrvUri uri, JSONObject parameter) throws WebDrvException {
		...
		{
			throw new WebDrvException(EWebDrvErrorCode.ELEMENT_NOT_INTERACTABLE, "Main window is hovered by some other modal window. Close all modal windows then click \"Forward\" button.");
		}

		Action a = this.getMainTree(session).getActions().getGoForwardAction();
		if(a.isEnabled())
			a.trigger();
		return null;
	}
