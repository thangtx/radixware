...
final class StrNumberMinimumValueSetting extends AbstractCheckableEditor<BigDecimal> implements IEditMaskEditorSetting{
    
    public StrNumberMinimumValueSetting(final IClientEnvironment environment, final QWidget parent) {
        super(environment, parent);
        final String title = environment.getMessageProvider().translate("EditMask", "Minimum:");
        setText(title);
        enable(false);
        setDefaultValue();
    ...
    public void loadFromXml(final org.radixware.schemas.editmask.EditMask editMask) {
        ...
        if(ems.isSetMinValue()) {
            final Double d = Double.valueOf(ems.getMinValue());
    ...
    public EEditMaskOption getOption() {
        return EEditMaskOption.STR_VC_NUMBER_MINIMUM;
    }
