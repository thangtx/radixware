...
final class NumMinimumValueSetting extends AbstractComplexCheckableSetting implements IEditMaskEditorSetting {
    private final ValNumEditor valueEditor;
    
    public NumMinimumValueSetting(final IClientEnvironment environment, final QWidget parent) {
        super(parent);
        
        valueEditor = new ValNumEditor(environment, this);
        addWidget(valueEditor, getOption());
        final String labelText = environment.getMessageProvider().translate("EditMask", "Minimum value:");
        setText(labelText);
        setDefaultValue();
    ...
    public void addToXml(final org.radixware.schemas.editmask.EditMask editMask) {
        if(isChecked()) {
            editMask.getNum().setMinValue(getValue());
        }
    }
    ...
    public void loadFromXml(final org.radixware.schemas.editmask.EditMask editMask) {
        ...
        if(editMaskNum.isSetMinValue()) {
            valueEditor.setValue(editMaskNum.getMinValue());
            enable(true);
        } else {
            setDefaultValue();
            enable(false);
    ...
    public EEditMaskOption getOption() {
        return EEditMaskOption.NUM_MINIMUM;
    }
