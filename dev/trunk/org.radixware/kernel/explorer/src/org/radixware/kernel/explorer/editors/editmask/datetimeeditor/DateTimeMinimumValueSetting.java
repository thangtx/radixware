...
final class DateTimeMinimumValueSetting extends AbstractCheckableEditor<Timestamp> implements IEditMaskEditorSetting {
    
    public DateTimeMinimumValueSetting(final IClientEnvironment environment, final QWidget parent) {
    ...
    public void addToXml(final EditMask editMask) {
        if(isChecked()) {
            ...
            editMask.getDateTime().setMinValue(value);
        }
    }
    ...
    public void loadFromXml(final EditMask editMask) {
        ...
        if(editMaskDateTime.isSetMinValue()) {
            final Calendar value = editMaskDateTime.getMinValue();
    ...
    public EEditMaskOption getOption() {
        return EEditMaskOption.DATETIME_MIN;
    }

    @Override
    public void setDefaultValue() {
        setValue(new Timestamp(1L));
    }
