...
final class GetTimeoutsCmd implements IWebDrvSessionCommand{
    ...
    public JSONObject execute(final WebDrvSession session, final WebDrvUri uri, final JSONObject parameter) throws WebDrvException {
        final JSONObject timeouts = session.getCapabilities().getTimeouts().getAsJsonObject();
        return new WebDrvSessionCommandResult(timeouts);
    }
}
