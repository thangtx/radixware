...
final class PointerUpAction extends PointerButtonAction {
    
    public PointerUpAction(final PointerInputSource inputSource, final int button){
    ...
    public void dispatch(final WebDrvSession session, int tickDuration, long tickStart) throws WebDrvException {
        ...
        if (btn!=null && state.isPressed(getButton())){
            WebDrvServer.postMouseRelease(btn);
            state.clearButton(btn);
        }
    }
}
