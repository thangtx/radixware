...
final class BooleanFalseValueSetting extends AbstractLabeledEditor<String>
        implements IEditMaskEditorSetting {


    public BooleanFalseValueSetting(final IClientEnvironment environment, final QWidget parent) {
        super(environment, parent, Qt.Orientation.Horizontal);
        final String title = environment.getMessageProvider().translate("EditMask", "False value:");
        setLabelText(title);
        setDefaultValue();
    }
    ...
    public EEditMaskOption getOption() {
        return EEditMaskOption.BOOLEAN_FALSE_VALUE;
    }

    @Override
    public void setDefaultValue() {
        final EditMaskBool em = new EditMaskBool();
        final String val = em.getFalseValueAsStr();
        setValue(val);
    }
