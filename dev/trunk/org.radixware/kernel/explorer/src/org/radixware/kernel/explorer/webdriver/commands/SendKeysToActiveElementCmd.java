...
public class SendKeysToActiveElementCmd extends GuiCmd {

    public static final String NAME = "SendKeysToActiveElementCmd";
    ...
    public JSONObject execute(WebDrvSession session, WebDrvUri uri, JSONObject parameter) throws WebDrvException {
        ...
        UIElementReference ref = null;
        if (w instanceof QAbstractItemView) {
            QAbstractItemView aiv = (QAbstractItemView) w;
            QModelIndex i = aiv.currentIndex();

            ref = session.getUIElements().getCellReference(aiv, i, false);
        } else {
            ref = session.getUIElements().getWidgetReference(w);
        }
