...
public class FileUtils {
    ...
    public static String readTextStream(final InputStream textStream, final String encoding) throws IOException {
        try (InputStreamReader reader = new InputStreamReader(textStream, encoding)) {
    ...
    public static void writeBytes(File file, byte[] bytes) throws IOException {
        try (OutputStream to = new FileOutputStream(file)) {
    ...
    public static void writeString(OutputStream to, String chars, String encoding) throws IOException {
        final byte[] bytes = chars.getBytes(encoding);
        writeBytes(to, bytes);
    }
    ...
    public static Map<String, byte[]> readJar(File file) throws IOException {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                byte[] bytes = getZipEntryByteContent(e, zip);
                if (bytes == null) {
                    bytes = new byte[0];
                }
    ...
    public static String findFirstChangeInJars(File oldJar, File newJar) throws IOException {
        final Map<String, byte[]> one = readJar(oldJar);
        final Map<String, byte[]> another = readJar(newJar);
