...
public abstract class TableUtils extends TestUtils {
    ...
    public ArrayList<DdsReferenceDef> createParentRefs(DdsTableDef child, ArrayList<DdsTableDef> parentTables) {
        ...
        for (int p = 0; p < count; p++) {
            ...
            assertNotNull(ref.findChildTable(ref));
            assertNotNull(ref.findParentTable(ref));
            assertEquals(parentTable.getId(), ref.findParentTable(ref).getId());
            assertEquals(child.getId(), ref.findChildTable(ref).getId());
    ...
    public ArrayList<DdsReferenceDef> createDetails(DdsTableDef parent, int detailsCount, int maxColumnCount) {
        ...
        for (int dtidx = 0; dtidx < detailsCount; dtidx++) {
            ...
            assertNotNull(ref.findChildTable(ref));
            assertNotNull(ref.findParentTable(ref));
            assertEquals(parent.getId(), ref.findParentTable(ref).getId());
            assertEquals(detail.getId(), ref.findChildTable(ref).getId());
