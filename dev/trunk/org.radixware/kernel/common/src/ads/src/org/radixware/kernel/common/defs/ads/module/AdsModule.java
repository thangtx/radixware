 ...
 *
 */
public class AdsModule extends Module implements IJavaSource, IJavaModule, IRadixdocProvider {
    ...
    boolean containsDefinitionInRepository(Id id, AdsDefinition[] result) {
        if (definitions == null) {
            ...
            AdsDefinition def = getDefinitions().findById(id);
            result[0] = def;
            return def != null;
        }
    }
    ...
    public char[][] getFactoryClassNameComponents() {
        char[][] components = JavaSourceSupport.getPackageNameComponents(this, JavaSourceSupport.UsagePurpose.SERVER_EXECUTABLE);
