 ...
 * @author npopov
 */
public class ChildRefExpItemRadixdoc extends SelectorExpItemRadixdoc {

    public ChildRefExpItemRadixdoc(AdsNodeExplorerItemDef source, Page page, DocumentOptions options) {
        super(source, page, options);
    }

    @Override
    protected void writeOverrideInfo(ContentContainer overview, Table genericAttrsTable) {
        super.writeOverrideInfo(overview, genericAttrsTable);
        AdsChildRefExplorerItemDef childRef = (AdsChildRefExplorerItemDef) source;
        getWriter().addStr2RefRow(genericAttrsTable, "DDS Reference", childRef.findChildReference(), childRef.findChildReference().getModule());
    }
}
