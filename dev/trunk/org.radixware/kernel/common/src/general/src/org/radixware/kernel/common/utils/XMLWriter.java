/**
 * XMLWriter.java - serialize an XML document. Written by David Megginson,
 * david@megginson.com NO WARRANTY! This class is in the public domain.
 *
 */
 ...
 *
 * <p>
 * is equivalent to the regular SAX2 ContentHandler method</p>
 *
 * <pre>
 ...
 *
 * <p>
 * The resulting document will look like this:</p>
 *
 * <pre>
 ...
 * <var>dataElement</var>, designed for writing elements that contain only
 * character data, so the code to generate the document could be shortened
 * to</p>
 *
 * <pre>
 * XMLWriter w = new XMLWriter();
 ...
 * adds newlines or indentation. If you insert three elements in a row, as
 * in</p>
 *
 * <pre>
 ...
 * needed, as in the following example:</p>
 *
 * <pre>
 * w.startDocument();
 ...
 * for selecting prefixes:</p>
 *
 * <ol> <li>the qualified name</li> <li>the {@link #setPrefix setPrefix}
 * method.</li> </ol>
 *
 * <p>
 ...
 * Namespace URI with the setPrefix method:</p>
 *
 * <pre>
 ...
 *
 * <p>
 * The default Namespace simply uses an empty string as the prefix:</p>
 *
 * <pre>
 ...
 * declarations, as in the following example:</p>
 *
 * <pre>
 * &lt;xml version="1.0" standalone="yes"?>
 *
 ...
 * predeclare Namespaces on the root element even if they are not used
 * there:</p>
 *
 * <pre>
 ...
 * not needed there, and can be inherited by its descendants:</p>
 *
 * <pre>
 * &lt;xml version="1.0" standalone="yes"?>
 *
 ...
 * qualified names appearing in attribute values or character data.</p>
 *
...
public class XMLWriter extends XMLFilterImpl {
     ...
     *
     * <p>
     * Write to the writer provided.</p>
     *
     ...
     *
     * <p>
     * Use the specified XML reader as the parent.</p>
     *
     ...
     * the writer, to make sure that no output is lost.</p>
     *
     * <p>
     ...
     * URI information in the writer or the selected output writer.</p>
     *
     * @see #flush
     */
    public void reset() {
     ...
     * but do not want to close the output stream.</p>
     *
     * <p>
     * This method is invoked automatically by the
     * {@link #endDocument endDocument} method after writing a document.</p>
     *
     * @see #reset
     */
    public void flush()
     ...
     * declared; to do that, use the {@link
     ...
     * document if it is not used on the root element.</p>
     *
     * <p>
     ...
     * by an end tag. Both a {@link #startElement
     ...
     * string for the qualified name. It invokes {@link
     * #startElement(String, String, String, Attributes)} directly.</p>
     *
     ...
     * invokes #startElement(String, String, String, Attributes)} directly.</p>
     *
     * @param localName The element's local name.
     * @exception org.xml.sax.SAXException If there is an error writing the
     * start tag, or if a handler further down the filter chain raises an
     ...
     * {@link #emptyElement(String, String, String, Attributes)} directly.</p>
     *
     * @param uri The element's Namespace URI.
     * @param localName The element's local name.
     * @exception org.xml.sax.SAXException If there is an error writing the
     ...
     * character array, then invokes {@link #characters(char[], int, int)}.</p>
     *
        ...
        for (int i = start; i < start + length; i++) {
            switch (ch[i]) {
                ...
                default:
                    write(ch[i]);
                /*
                 * disable escaping characters with ascii-code > 127; RADIX-6390
                 * if (ch[i] > '\u007f') { write("&#");
                 * write(Integer.toString(ch[i])); write(';'); } else {
                 * write(ch[i]); }
                 */
            }
        }
