...
class IdTagTranslator<T extends IdTag> extends SqmlTagTranslator<T> {

    public IdTagTranslator(IProblemHandler problemHandler) {
    ...
    public void translate(T tag, CodePrinter cp) {
        ...
        final String idAsStr = target.getId().toString();
        cp.printStringLiteral(idAsStr);
    }
}
