/*This 2 libs are in one file cause d3 is bigger than metricsgraphics 
and due to async download d3 will load later than metricsgraphics 
but we need it before metricgraphics inited*/
...
// metricsgraphics.js v.2.11.0 Mozilla Public License, version 2.0
...
//Fix for xax_count value set
function mg_get_auto_time_frame(diff) {
  var time_frame;
  if (mg_sec_diff(diff)) {
    time_frame = 'less-than-a-day';
  } else if (diff / (60 * 59) <= 24) {
    time_frame = 'less-than-a-day';
  } else {
    time_frame = 'many-days';
  }
...
//Fix for xax_count value set
function mg_get_auto_time_format(utc, diff) {
  var main_time_format;
  if (mg_sec_diff(diff)) {
    main_time_format = MG.time_format(utc, '%H:%M:%S');
  } else if (diff / (60 * 59) <= 24) {
    main_time_format = MG.time_format(utc, '%H:%M');
  } else if (mg_many_days(diff)) {
    main_time_format = MG.time_format(utc, '%d-%b');
  } else if (mg_many_months(diff)) {
    main_time_format = MG.time_format(utc, '%b');
  } else {
    main_time_format = MG.time_format(utc, '%Y');
...
function mg_process_time_format(args) {
  ...
  if (args.time_series) {
    ...
    if (args.x_axis_date_autoType == true) { //Fix for xax_count value set
      intervalDiff = (args.processed.max_x - args.processed.min_x) / (1000 * args.processed['x_ticks'].length);
      time_frame = mg_get_auto_time_frame(intervalDiff);
      main_time_format = mg_get_auto_time_format(args.utc_time, intervalDiff);
    } else {
