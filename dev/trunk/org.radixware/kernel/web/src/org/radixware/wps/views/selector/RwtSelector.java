...
public abstract class RwtSelector extends Container implements ISelector, IModificationListener {
    ...
    private static class EditorSpace extends VerticalBox implements IEditorSpace {
...
//        splitter.addSplitterListener(new Splitter.SplitterListener() {
//
//            @Override
//            public void partChanged(int part, float currentValue) {
//                if (filtersAndSortingsToolBar != null && part == 0) {
//                    filterAntOrderPart = currentValue;
//                }
//            }
//        });
        splitter.setTop(0);
        ...
        final SelectorController.UIController uiController = new SelectorController.UIController() {
            ...
            @Override
            protected void showBatchOperationResult(AbstractBatchOperationResult result, String message) {
                throw new UnsupportedOperationException("Not supported yet.");
            }
        };
        ...
        this.actions = new Actions(controller) {
            ...
            protected Action createAction(ClientIcon icon, String title) {
                RwtAction action = new RwtAction(env, icon);
...
//    }
    ...
    public void open(Model model) {
        ...
        this.controller.open(model, null);
    }
    ...
    public boolean close(boolean forced) {
        if (getSelectorWidget() instanceof RwtSelectorGrid) {
            ((RwtSelectorGrid) getSelectorWidget()).storeHeaderSettings();
            ((RwtSelectorGrid) getSelectorWidget()).actions.close();
        } else if (getSelectorWidget() instanceof RwtSelectorTree) {
            ((RwtSelectorTree) getSelectorWidget()).storeHeaderSettings();
        }        
        if (controller.close(forced)) {
            customHandlers.clear();
            final ISelectorWidget widget = getSelectorWidget();
            if (widget instanceof UIObject)
                splitter.remove((UIObject)widget);
            return true;
        } else {
