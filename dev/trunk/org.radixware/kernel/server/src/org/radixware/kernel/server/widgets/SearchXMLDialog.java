...
public class SearchXMLDialog {

	private static final class SearchComboBox extends JComboBox {

		private static final long serialVersionUID = -6969392466123447297L;
	...
	private final class SearchMachine {
		...
		public SearchMachine(JTree tree) {
			this.tree = tree;
		...
		public void find(String searchingString) {
			...
			int start = tree.getMaxSelectionRow() >= 0 ? tree.getMaxSelectionRow() : 0;
			if (isRegex) {
				searchingString = "(.)*" + searchingString + "(.)*";
				if (isForward) {
					int r = start, end = tree.getRowCount();
					try {
						for (r = start; r < end; r++) {
					...
					if (r != end) {
						tree.firePropertyChange(tree.getName(), -1, r);
					} else {
		...
		private boolean findStringAtRow(String string, int row) {
			try {
				if (!isCaseSensitive) {
					return ((String) ((DefaultMutableTreeNode) tree.getPathForRow(row).getLastPathComponent()).getUserObject()).toLowerCase().indexOf(string) >= 0;
				} else {
					return ((String) ((DefaultMutableTreeNode) tree.getPathForRow(row).getLastPathComponent()).getUserObject()).indexOf(string) >= 0;
				}
		...
		private boolean findRegexAtRow(String regex, int row) throws PatternSyntaxException {
			try {
				return ((String) ((DefaultMutableTreeNode) tree.getPathForRow(row).getLastPathComponent()).getUserObject()).matches(regex);
			} catch (RuntimeException e) {
				return false;
			}
		}
	...
	public SearchXMLDialog(Object parent, JTree tree) {
		this.parent = parent;
		this.tree = tree;
	}
	...
	private JPanel createDirectionRadioButtons() {
		...
		forwardRadioButton.setSelected(true); //������ ��������� � SearchMachine.isForward
	...
	private JPanel createButtonBar() {
		...
		panel.setBorder(new EmptyBorder(3, 0, 0, 20));
	...
	public void show() {
		if (dialog == null) {
			searchMachine = new SearchMachine(tree);
			while (parent != null && !(parent instanceof Frame)) {
				parent = ((Component) parent).getParent();
			}
			dialog = new JDialog((Frame) parent, Messages.TITLE_FIND, true);
