...
public final class ProgressDialogResource extends Resource {
	...
	/**
     * Посылает клиенту запрос на отображение окна для показа прогресса выполнения процессов на сервере
     * @param caption   - может быть null - в этом случае клиент должен отобразить
     *                    стандартный заголовок ("Пожалуйста, подождите...")
     * @param showTrace - отображать трассу, если хотя бы для одного из отображаемых 
     *                    процессов трасса нужна, должен быть равен true
     */        
    ...
    public Process startNewProcess(final String title, final boolean cancellable, final boolean showPercent ) throws ResourceUsageException, ResourceUsageTimeout, InterruptedException {
        if( firstProcess == null || firstProcess.closed )
           firstProcess = new Process(arte, caption, showTrace, title, cancellable, showPercent, true );
        else
    ...
    public static final class Process {
         ...
         * @param caption - заголовок диалога, используется только если isFirst = true   
         * @param showTrace - настройка диалога, используется только если isFirst = true   
         * @param title       - заголовок текущего процесса, может быть null
         * @param cancellable - может ли быть кнопка "Cancel" при выполнении текущего процесса 
         * @param showPercent - если false - ProgressBar будет без фиксированной max границы
         * @param isFirst - первый ли процесс в этом диалоге (если первый то в запросе будут переданы настройки самого диалога)   
         * @throws ResourceUsageTimeout 
         * @throws ResourceUsageException 
         * @throws InterruptedException 
         */        
        protected Process(final Arte arte, final String caption, final boolean showTrace, final String title, final boolean cancellable, final boolean showPercent, final boolean isFirst )
        ...
        throws ResourceUsageException, ResourceUsageTimeout, InterruptedException{
            checkClosed();
            if ( 
                lastDeliveredSetMillis + SET_DELIVERY_PERIOD_MILLIS > System.currentTimeMillis() && // to litte time passed
        ...
        /** Пример формирования трассы: 
         *  TraceType trace = TraceType.Factory.newInstance();
         *  org.radixware.schemas.eas.TraceType.Item[] traceItems = new org.radixware.schemas.eas.TraceType.Item[1];
         *  traceItems[0] = org.radixware.schemas.eas.TraceType.Item.Factory.newInstance();
         *  traceItems[0].setLevel( TraceLevelEnum.DEBUG );
         *  traceItems[0].setStringValue("Debug Message");
         *  trace.setItemArray( traceItems );
         * @throws ResourceUsageTimeout 
         * @throws ResourceUsageException 
         * @throws InterruptedException 
