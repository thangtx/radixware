...
class DiffFileTable implements MouseListener, ListSelectionListener, AncestorListener {
    ...
    private class DiffTableCellRenderer extends DefaultTableCellRenderer {
        ...
        // RADIX
                String htmlDisplayName = getHtmlDisplayName(nodes[modelRow],
                                                      isModified(modelRow),
                                                      isSelected);
                if (SvnModuleConfig.getDefault().isExcludedFromCommit(nodes[modelRow].getSetup().getBaseFile().getAbsolutePath())) {
        // RADIX
                    htmlDisplayName = "<s>" + (htmlDisplayName == null ? RadixFileUtil.getFileDisplayName(nodes[modelRow].getSetup().getBaseFile()) : htmlDisplayName) + "</s>"; //NOI18N
                }
        ...
        // RADIX
                    value = RadixFileUtil.getFileDisplayName(nodes[modelRow].getSetup().getBaseFile());
                }
            }
        ...
        // RADIX
        private String getHtmlDisplayName(Node node, boolean fileModified, boolean selected) {
        // RADIX
        ...
        // RADIX
                                    : RadixFileUtil.getFileDisplayName(((DiffNode)node).getSetup().getBaseFile());
        // RADIX
