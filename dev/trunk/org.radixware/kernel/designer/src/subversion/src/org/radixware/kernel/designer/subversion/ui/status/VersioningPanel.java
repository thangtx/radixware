 ...
 */
package org.radixware.kernel.designer.subversion.ui.status;
...
//        reScheduleRefresh(0);
    ...
    private SyncFileNode[] getNodes(Context context, int includeStatus) {
        File[] fnodes = SvnUtils.getModifiedFiles(context, includeStatus);
        SyncFileNode[] nodes = new SyncFileNode[fnodes.length];
        for (int i = 0; i < fnodes.length; i++) {
            if (Thread.interrupted()) {
                return null;
            }
            SvnFileNode fnode = new SvnFileNode(fnodes[i]);
    ...
    /**
     * Performs the "cvs commit" command on all diplayed roots plus "cvs add"
     * for files that are not yet added.
     */
    ...
    /**
     * Refreshes statuses of all files in the view. It does that by issuing the
     * "svn status -u" command, updating the cache and refreshing file nodes.
     */
    ...
    /**
     * Programmatically invokes the Refresh action. Connects to repository and
     * gets recent status.
     */
    ...
    private void refreshStatuses() {
        ...
        if (repository == null) {
            ...
            if (allUnmanaged) {
                Subversion.LOG.log(Level.INFO, null, new IllegalArgumentException("VersioningPanel.refreshStatuses: null repositoryUrl for " + context.getRootFiles()[0].getAbsolutePath()));
            }
        }
    ...
    /**
     * Shows Diff panel for all files in the view. The initial type of diff
     * depends on the sync mode: Local, Remote, All. In Local mode, the diff
     * shows CURRENT <-> BASE differences. In Remote mode, it shows BASE<->HEAD
     * differences.
     */
        ...
        public void run() {
            ...
            try {
                ...
                if (Thread.currentThread().isInterrupted()) {
                    return; // due to cancelRefresh() method
                }
                throw e;
            }
        }
    ...
    /**
     * Hardcoded toolbar layout. It eliminates need for nested panels their look
     * is hardly maintanable accross several look and feels (e.g. strange
     * layouting panel borders on GTK+).
     *
     * <p>
     * It sets authoritatively component height and takes "prefered" width from
     * components itself.
     *
     */
...
//                }
        ...
        /**
         * Computes vertical toolbar components height that can used for layout
         * manager hinting.
         *
    ...
    private void tgbAllActionPerformed(java.awt.event.ActionEvent evt) {
        onDisplayedStatusChanged();
    }
    ...
    private void tgbRemoteActionPerformed(java.awt.event.ActionEvent evt) {
        onDisplayedStatusChanged();
    }

    private void tgbLocalActionPerformed(java.awt.event.ActionEvent evt) {
        onDisplayedStatusChanged();
    }
