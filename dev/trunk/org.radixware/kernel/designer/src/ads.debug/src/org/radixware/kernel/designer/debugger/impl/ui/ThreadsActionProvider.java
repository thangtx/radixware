...
public class ThreadsActionProvider implements NodeActionsProvider {
    ...
    private final Action MAKE_CURRENT_ACTION = Models.createAction("Make current", new Models.ActionPerformer() {
        ...
        public void perform(Object[] nodes) {
            if (nodes.length == 1 && (nodes[0] instanceof ThreadWrapper)) {
                ThreadWrapper w = (ThreadWrapper) nodes[0];
                w.setCurrent();
            }
        }
    }, Models.MULTISELECTION_TYPE_EXACTLY_ONE);

    public ThreadsActionProvider(ContextProvider lookupProvider) {
    }
