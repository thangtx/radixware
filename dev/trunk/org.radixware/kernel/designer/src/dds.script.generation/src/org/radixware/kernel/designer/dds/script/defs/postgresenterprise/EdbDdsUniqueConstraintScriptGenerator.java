...
public class EdbDdsUniqueConstraintScriptGenerator extends EdbDdsConstraintScriptGenerator<DdsUniqueConstraintDef> {
    ...
    protected EdbDdsUniqueConstraintScriptGenerator() {
    }
    ...
    public boolean isModifiedToDrop(DdsUniqueConstraintDef oldUc, DdsUniqueConstraintDef newUc) {
        ...
        EdbDdsIndexScriptGenerator indexScriptGenerator = EdbDdsIndexScriptGenerator.Factory.newInstance();
    ...
    public void getCreateScript(CodePrinter cp, DdsUniqueConstraintDef uc, IScriptGenerationHandler handler) {
        ...
        final DdsIndexDef idx = uc.getOwnerIndex();
        final DdsTableDef table = idx.getOwnerTable();
        final String tableDbName = table.getDbName();

        if (!(idx instanceof DdsPrimaryKeyDef)) {
            cp.println("-- UNIQUE: "+uc.getClass()+" idx:"+idx.getClass());
    ...
    public void afterModifyPkScript(CodePrinter cp, DdsPrimaryKeyDef pk) {
        ...
        for (AdsUserPropertyDef prop : requestor.getRefsFrom(table)) {
            ...
            if (UserPropsRequestor.isUserFuncDef(prop)) {
                if (!isUserFunc) {
                    ...
                    isUserFunc = true;
                    cp.println("alter table RDX_USERFUNC drop constraint FK_RDX_USERFUNC_UPVALREF").printCommandSeparator().println();
                }

                printPkModifyScriptHeader(cp, table);
                cp.println("update RDX_USERFUNC set UPOWNERPID = UPOWNERPID /* modify this query for correct PID conversion */ where UPDEFID = '").print(prop.getId())
                  .print("' and UPOWNERENTITYID = '").print(table.getId()).print("'").printCommandSeparator().println();
            }

            printPkModifyScriptHeader(cp, table);
            cp.println("update RDX_").print(tableName).print(" set OWNERPID = OWNERPID /* modify this query for correct PID conversion */ where DEFID = '").print(prop.getId())
              .print("' and OWNERENTITYID = '").print(table.getId()).print("'").printCommandSeparator().println();
        }
        if (isUserFunc) {
            ...
            cp.printCommandSeparator().println();
        }
        ...
        for (AdsUserPropertyDef prop : requestor.getRefsTo(table)) {
            ...
            printPkModifyScriptHeader(cp, table);
            cp.println("update RDX_UPVALREF set VAL = VAL /* modify this query for correct PID conversion */ where DEFID = '").print(prop.getId()).print("' and OWNERENTITYID = '")
              .print(ownerTable.getId()).print("'").printCommandSeparator().println();
        }
    ...
    public static final class Factory {
        ...
        public static EdbDdsUniqueConstraintScriptGenerator newInstance() {
            return new EdbDdsUniqueConstraintScriptGenerator();
        }
    }
}
