...
public class OraDdsPlSqlBodyScriptGenerator extends OraDdsPlSqlPartScriptGenerator<DdsPlSqlBodyDef> {

    public OraDdsPlSqlBodyScriptGenerator() {
    }
        ...
        for (DdsPlSqlObjectItemDef item : plSqlObject.getBody().getItems()) {
            ...
            if (item instanceof DdsFunctionDef) {
                ...
                if (function.isGeneratedInDb()) {
                    cp.print("\n\n");
                    OraDdsFunctionScriptGenerator.getScript(cp, function, handler);
                }
                ...
                if (function.isGeneratedInDb()) {
                    cp.print("\n\n");
                    OraDdsFunctionScriptGenerator.getHeaderScript(cp, function);
                    cp.print(';');
                }
            } else if (item instanceof DdsCustomTextDef) {
                cp.print("\n\n");
                final DdsCustomTextDef customText = (DdsCustomTextDef) item;
                OraDdsCustomTextScriptGenerator.getScript(cp, customText);
            } else {
    ...
    public static final class Factory {
        ...
        public static OraDdsPlSqlBodyScriptGenerator newInstance() {
            return new OraDdsPlSqlBodyScriptGenerator();
        }
    }
}
