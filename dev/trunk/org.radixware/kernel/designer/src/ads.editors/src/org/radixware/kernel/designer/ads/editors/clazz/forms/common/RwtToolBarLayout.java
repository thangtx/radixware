...
public class RwtToolBarLayout implements Layout {

    public RwtToolBarLayout() {
    }
    ...
    public void justify(Widget widget) {
        synchronized (LayoutProcessor.LOCK) {
            final AdsRwtWidgetDef node = (AdsRwtWidgetDef) ((BaseWidget) widget).getNode();
            if (RwtToolBarItem.getOrientation(node) == EOrientation.Vertical) {
    ...
    private void justifyHorizontal(AdsRwtWidgetDef node, Widget widget) {
        final Rectangle geometry = ((BaseWidget) widget).getLayoutGeometry();
        Point start = getStartPoint(geometry, EOrientation.Horizontal);

        for (final BaseWidget wdg : RwtToolBarLayoutProcessor.getWidgets(node, (GraphScene) widget.getScene())) {
            final Rectangle calcRect = calcHorizontalRect(start, geometry,
                    RwtActionItem.calcActionSize(getFontMetrics(widget.getScene()), (AdsRwtWidgetDef) wdg.getNode()));
    ...
    private void justifyVertical(AdsRwtWidgetDef node, Widget widget) {
        final Rectangle geometry = ((BaseWidget) widget).getLayoutGeometry();
        ...
        for (final BaseWidget wdg : RwtToolBarLayoutProcessor.getWidgets(node, (GraphScene) widget.getScene())) {
            ...
            start = new Point(start.x, start.y + calcRect.height + RwtToolBarLayoutProcessor.INTERVAL);
            if (!wdg.isSelfContained()) {
                wdg.setGeometry(calcRect);
                wdg.saveGeometry();
            }
