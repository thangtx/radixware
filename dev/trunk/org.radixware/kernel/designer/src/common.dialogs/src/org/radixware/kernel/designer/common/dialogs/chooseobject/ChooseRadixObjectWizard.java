...
class ChooseRadixObjectWizard implements IChooseRadixObjectModalDisplayer {

    private final ChooseRadixObjectCfg cfg;
    private final boolean multipleSelectionAllowed;
    List<RadixObject> result = null;

    public ChooseRadixObjectWizard(ChooseRadixObjectCfg cfg, boolean multipleSelectionAllowed) {
        this.cfg = cfg;
        this.multipleSelectionAllowed = multipleSelectionAllowed;
    }

    private final class ChooseDefinitionStep extends Step<ChooseRadixObjectPanel> {

        private final int currentStepNumber;
        private final RadixObject commonOwner;

        public ChooseDefinitionStep(int currentStepNumber, RadixObject commonOwner) {
        ...
        @Override
        protected ChooseRadixObjectPanel createVisualPanel() {
            final ChooseRadixObjectPanel chooseDefinitionPanel = new ChooseRadixObjectPanel();
        ...
        public Step createNextStep() {
            final RadixObject nextCommonOwner = getVisualPanel().getSelected();
            return new ChooseDefinitionStep(currentStepNumber + 1, nextCommonOwner);
        }

        private boolean isAlgoProp() {
            final RadixObject def = getVisualPanel().getSelected();
            return (cfg.isForAlgo() && (def instanceof AdsVarObject
