...
class JmlCCOperations {

    static int nextCamelCasePosition(JTextComponent textComponent) throws BadLocationException {
        int implRet = nextCamelCasePositionImpl(textComponent);
        //test for tag
        if (textComponent instanceof ScmlEditorPane) {
            ScmlEditorPane pane = (ScmlEditorPane) textComponent;
            TagMapper tm = pane.getScmlDocument().getTagMapper();
            if (tm != null) {
                TagBounds tb = tm.findContainingBounds(implRet);
                if (tb != null && tb.getEndOffset() > implRet && tb.getBeginOffset() != implRet) {
                    return tb.getEndOffset();
                }
            }
        }
        return implRet;
    ...
    static int nextCamelCasePositionImpl(JTextComponent textComponent) throws BadLocationException {
    ...
    static int previousCamelCasePosition(JTextComponent textComponent) throws BadLocationException {
        int implRet = previousCamelCasePositionImpl(textComponent);
        //test for tag
        if (textComponent instanceof ScmlEditorPane) {
            ScmlEditorPane pane = (ScmlEditorPane) textComponent;
            TagMapper tm = pane.getScmlDocument().getTagMapper();
            if (tm != null) {
                TagBounds tb = tm.findContainingBounds(implRet);
                if (tb != null && tb.getBeginOffset() < implRet) {
                    return tb.getBeginOffset();
                }
            }
        }
        return implRet;
    ...
    static int previousCamelCasePositionImpl(JTextComponent textComponent) throws BadLocationException {
        SyntaxSupport syntaxSupport = Utilities.getSyntaxSupport(textComponent);
        if (syntaxSupport == null) {
            // no syntax support available :(
            return -1;
        ...
        if (textComponent instanceof ScmlEditorPane) {
            ...
            if (tm != null) {
                ...
                if (tb != null && tb.getBeginOffset() < offset) {
                    return tb.getBeginOffset();
                }
            }
        }
