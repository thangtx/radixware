create or replace package body RDX_ADS_META as
	...
	is
	    versionNum integer := RDX_Arte.getVersion();
	begin
	    if pDefId = pDomainId then
	        return 1;
	    end if;
	    for cur in (select DOMAINID domainId from RDX_DEF2DOMAIN where VERSIONNUM = versionNum and DEFID = pDefId) loop
	...
	begin
	    ...
	    for cur in (select /*+ INDEX(RDX_CLASSANCESTOR PK_RDX_CLASSANCESTOR) */ ANCESTORID from RDX_CLASSANCESTOR where VERSIONNUM = versionNum and CLASSID = pClsId) loop
	...
	begin
	    for cur in (select DOMAINID enumItemDomainId from RDX_ENUMITEM2DOMAIN where 
	        VERSIONNUM = versionNum and ENUMID = pEnumId and
