create or replace package body RDX_Trace as
	...
	is
	    ...
	    wordsArr RDX_Array.ARR_STR := RDX_Array.fromStr(pWords);
	    result clob := null;
	    word clob := null;
	begin
	    if pBundleId is NULL or pStringId is NULL then
	        if wordsArr is not NULL and wordsArr.count > 0 then
	            return wordsArr(1);
	        end if;
	        return null;
	    else
	        select STRINGVALUE into result from RDX_EVENTCODEMLS
	        where VERSIONNUM = version and BUNDLEID = pBundleId and STRINGID = pStringId and LANGUAGE = pLanguage;
	        
	        for i in 1 .. 9 loop
	            if wordsArr is not NULL and wordsArr.count >= i then
	                word := wordsArr(i);
	            else
	                word := 'null';
	            end if;
	            result := replace(result, '%' || i, word);
	        end loop;
	    end if;
	    return result;
	exception
